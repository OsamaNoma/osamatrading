    ...
    if sorted_tickers:
        ...
        if selected_ticker != None and pct_change != None:
            ...
            if balance >= 20:
                ...
                print("Buy order placed successfully"),Tb.send_message("Buy order successfully in Binance")
                print("Entery price:",str(avgPrice))
                detail, tp, trigger, sl = po.oco_order_sell(symbol = tkr, avg_price = avgPrice, pct_sl = pct_change)
                print("OCO order placed successfully"),Tb.send_message("OCO order placed successfully")
